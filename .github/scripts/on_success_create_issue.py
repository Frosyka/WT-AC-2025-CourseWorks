    ...
    while url:
        ...
        try:
            with urllib.request.urlopen(req, timeout=30) as resp:
                if resp.status != 200:
                    LOG.warning('Failed to fetch PR files: %s', resp.status)
                    break
            ...
            LOG.warning('Failed to fetch PR files: %s %s', e.code, e.read().decode('utf-8', 'ignore'))
            break
    return files
...
def ensure_label(repo: str, pr_number: str, headers: dict, label: str):
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            if resp.status == 200:
                names = [x.get('name') for x in json.loads(resp.read().decode('utf-8'))]
                if label in names:
                    return
...
def create_issue(repo: str, headers: dict, title: str, body: str) -> int:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            LOG.info('create_issue status=%s', resp.status)
            if resp.status in (200, 201):
                return json.loads(resp.read().decode('utf-8')).get('number', 0)
            return 0
    except urllib.error.HTTPError as e:
        LOG.info('create_issue status=%s', e.code)
        return 0
...
def comment_pr(repo: str, pr_number: str, headers: dict, body: str):
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            LOG.info('comment_pr status=%s', resp.status)
    except urllib.error.HTTPError as e:
        LOG.info('comment_pr status=%s', e.code)
